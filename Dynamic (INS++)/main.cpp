...
void Graph::insert(int u, int v)
{
	...
	if (rd[u] <= PA - 2)
		...
		push_edge(u, v);
	else if (top.find(u) == top.end() || top.find(v) == top.end())
	{
		int ans = -1;
		queue<int> Q;
		unordered_set<int> vis;
		unordered_map<int, int> p;
		if (top.find(u) == top.end()) { Q.push(u); vis.insert(u); }
		if (top.find(v) == top.end()) { Q.push(v); vis.insert(v); }
		while (!Q.empty())
		{
			...
			for (auto ne : e[now])
			{
				...
				if (top.find(ne.from) != top.end()) continue;
				p[ne.from] = now;
				vis.insert(ne.from);
				Q.push(ne.from);
			}
	...
	{
		unordered_set<int> vis; unordered_map<int, int> p;
		queue<int> Q; Q.push(u); Q.push(v); vis.insert(u); vis.insert(v);
		int ans = -1;
		while (!Q.empty())
		{
			int now = Q.front(); Q.pop();
...
void Graph::delet(int u, int v)
{
	...
	if (rd[v] == PA || rd[v] <= (PA - 2))
		pull_edge(u, v);
	else if (rd[v] == (PA - 1))
	{
		sum++;
		unordered_set<int> vis; unordered_map<int, int> p;
		queue<int> Q; Q.push(v); vis.insert(v);
		int ans = -1;
		while (!Q.empty())
		{
			int now = Q.front(); Q.pop();
			...
			for (auto ne : e[now])
			{
				...
				if (rd[ne.to] < (PA - 1)) continue;
				if (vis.find(ne.to) != vis.end()) continue;
				p[ne.to] = now;
				vis.insert(ne.to);
				Q.push(ne.to);
			}
