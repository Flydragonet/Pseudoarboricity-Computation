...
void Graph::output(char* address) {
	FILE* output_address = fopen(address, "w");
	for (unsigned i = 0; i < m; i++) {
		unsigned from = e[i].t1 == e[i].to ? e[i].t2 : e[i].t1;
		fprintf(output_address, "%d %d\n", from, e[i].to);
	}
}
...
int main(int argc, char** argv)
{
	...
	char graph_name[100]; strcpy(graph_name, argv[1]);
	double runtime;
