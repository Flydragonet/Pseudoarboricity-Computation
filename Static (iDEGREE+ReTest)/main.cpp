...
void Graph::approximate_orientation()
{
	...
	unsigned cnt = 0;
	unsigned Enum = m; double rho = 1.0 * m / n;
	...
	{
		unsigned now = order[j];
		if (core[now] < p0) break;
		nodes.insert(now);
		for (unsigned i = 0; i < adj_length[now]; i++)
		{
			Edge& ne = e[adj[now][i]];
...
void Graph::exact_orientation()
{
	...
	unsigned p_upper = get_max_d() + 1, p_lower = p0;
...
int main()
{
	...
	while (fscanf(dataset, "%s", &graph_name) == 1)
	{
		...
		{
			TEST::p0 = G.get_max_d();
			printf("Maximum indegree = %u\nRounding up approximate density = %u\n", TEST::p0, G.p0);
